...
const galleryTrack = galleryPreview.querySelector('.gallery-track')
...
const scrollGallery = direction => {
  const reducedMotion = window.matchMedia('(prefers-reduced-motion: reduce)').matches
  const distance = Math.min(galleryTrack.clientWidth * .8, 480)

  galleryTrack.scrollBy({ left: distance * direction, behavior: reducedMotion ? 'auto' : 'smooth' })
}
